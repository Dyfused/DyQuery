    ...
    try:
        if len(args) == 1:
            ...
            if dyuserinfo := await sql_session.get(dyUserInfo, event.get_user_id()):
                ...
                await best20.finish(
                    "您还未绑定Dynamite用户名。请使用 /dybind <用户名> 来绑定。",
                    reply_message=True,
                )

        await best20.send(f"正在查询 {username} 的 Best20 成绩", reply_message=True)
        image_b64 = await wrapper_draw_best_20(
            uuid, username, timeout_seconds=max(20, config.http_timeout_seconds * 3)
        )
        ...
        if "NO_PLAY_RECORD" in msg:
            await best20.finish(
                "查询失败: The user does not have any play records.", reply_message=True
            )
        elif "USER_NOT_FOUND" in msg:
            await best20.finish(
                "查询失败: 500 USER_NOT_FOUND: User does not exist.", reply_message=True
            )
        else:
...
async def handle_best20_discord(
    bot: Bot,
    event: Event,
    sql_session: async_scoped_session,
    username: CommandOption[Optional[str]],
):
    """
    ...
    try:
        ...
        image_data = await wrapper_draw_best_20_binary(
            uuid, usr, timeout_seconds=max(20, config.http_timeout_seconds * 3)
        )
        ...
        await best20_discord.finish(
            DiscordMessageSegment.attachment(file="image.jpg", content=raw)
        )
    except asyncio.TimeoutError as timeout_exc:
        await best20_discord.finish(
            f"Query failed: {timeout_exc}\n Please try again later."
        )
    except BombException as exc:
        # 原逻辑：BombException 一律按 USER_NOT_FOUND 处理
        # 现在：根据 utils.py 里抛出的错误文本做细分（不新增错误类，保持原异常体系）
        ...
        if "NO_PLAY_RECORD" in msg:
            ...
            await best20_discord.finish(
                "Query failed: 500 USER_NOT_FOUND: User does not exist."
            )
        else:
